...
public class TracksDAO extends CassandraData {
  ...
  public static List<TracksDAO> listSongsByGenre(String genre, int num_tracks) {
    ...
    String queryText = "SELECT * FROM track_by_genre WHERE genre = ? limit " + num_tracks;
    PreparedStatement preparedStatement = getSession().prepare(queryText);
   ...
   */
  public void star() {
	  getSession().execute("update track_by_genre using ttl 30 set stars = 5 where genre = ? and artist = ? and track = ? and track_id = ?", this.genre, this.artist, this.track, this.track_id);
	  getSession().execute("update track_by_artist using ttl 30 set stars = 5 where artist = ? and track = ? and track_id = ?", this.artist, this.track, this.track_id);
	  getSession().execute("update track_by_id using ttl 30 set stars = 5 where track_id = ?", this.track_id);
  }
